...
final class UserGuideSettingsViewController: NSViewController {
    ...
    private func buildGuideContent() -> NSAttributedString {
        ...
        body("A real-time loudness normaliser approximating ITU-R BS.1770-4 short-term (3-second) integrated loudness. A measurement tap runs a two-stage K-weighting filter (a high-shelf pre-filter followed by a high-pass), accumulates block mean-square power into a 3-second circular FIFO, and excludes any block quieter than a −70 dBFS gate (raised to −60 dBFS when the Dialogue Gate option is enabled, so that silence between lines of dialogue doesn't skew the estimate). The gain correction uses asymmetric attack/release time constants for more natural response:")
        code("gain_dB = target_LUFS − measured_LUFS\nα_attack  = exp(−blockSize / (fs · attackSeconds))   (faster, 0.3–5.0 s)\nα_release = exp(−blockSize / (fs · releaseSeconds))  (slower, 1.0–10.0 s)\nα = (targetGain < smoothedGain) ? α_attack : α_release\nsmoothedGain[n] = α·smoothedGain[n−1] + (1−α)·10^(gain_dB/20)")
        body("The asymmetric smoothing means gain decreases quickly when the program gets louder (preventing a loud scene's onset from riding at full level for long), but increases slowly when the program gets quieter (avoiding sudden recovery into a loud scene's reverb tail). Target range −24 to −10 LUFS (default −16 LUFS). Correction is clamped to a configurable ceiling (default ±12 dB) to keep the stage from chasing measurement noise or over-correcting unusually wide swings. Content with scene-to-scene loudness variation wider than the ceiling will be partially, not fully, corrected in its most extreme moments — raise the ceiling if that's a recurring issue with specific content, but note that wider settings mean larger, more noticeable gain changes when they do engage. This stage measures the widened, but not-yet-gain-corrected, signal — the gain is applied first, then the new measurement is taken, which avoids feedback oscillation between measurement and correction.")
