...
final class OversamplingProcessor {
    ...
    private static let phases: Int = upsampleFactor
    ...
    func upsample(input: [UnsafePointer<Float>], frameCount: Int, output: UnsafeMutablePointer<Float>) {
        guard enabled else {
            ...
            for ch in 0..<channelCount {
                let inPtr = input[ch]
                var outPtr = output.advanced(by: ch * frameCount * upsampleFactor)
                for i in 0..<frameCount {
                    outPtr.pointee = inPtr[i]
                    outPtr = outPtr.advanced(by: upsampleFactor)
                }
            }
            return
        }
        ...
        for ch in 0..<channelCount {
            ...
            for i in 0..<frameCount {
                ...
                for phase in 0..<upsampleFactor {
                    let coeff = upsampleCoefficients[phase]
                    var acc: Float = 0

                    // FIR filter using polyphase coefficients
                    for tap in 0..<filterTaps {
                        let stateIdx = (i * upsampleFactor + phase - tap) % filterTaps
                    ...
                    // Update state
                    for tap in stride(from: filterTaps - 1, through: 1, by: -1) {
    ...
    func downsample(input: UnsafePointer<Float>, frameCount: Int, output: [UnsafeMutablePointer<Float>]) {
        guard enabled else {
            ...
            for ch in 0..<channelCount {
                let inPtr = input.advanced(by: ch * frameCount * upsampleFactor)
                let outPtr = output[ch]
                for i in 0..<frameCount {
                    outPtr[i] = inPtr[i * upsampleFactor]
                }
            }
            return
        }
        ...
        for ch in 0..<channelCount {
            ...
            for i in 0..<frameCount {
                ...
                for phase in 0..<upsampleFactor {
                    ...
                    let sampleIdx = i * upsampleFactor + phase

                    for tap in 0..<filterTaps {
                        let stateIdx = (sampleIdx - tap) % filterTaps
                        if stateIdx >= 0 && stateIdx < state.count {
                            acc += state[stateIdx] * coeff[tap]
                        }
                    }
                    ...
                    if sampleIdx < state.count {
                        state[sampleIdx % filterTaps] = inPtr[sampleIdx]
                    }
                }
    ...
    private func generateCoefficients() {
        ...
        let cutoff = 0.5 / Double(upsampleFactor)
        let nyquist = 0.5

        upsampleCoefficients = (0..<phases).map { phase in
            (0..<filterTaps).map { tap in
                let t = Double(tap) - Double(filterTaps - 1) / 2.0
                let n = t + Double(phase) / Double(phases)
                // Sinc windowed by Hamming
                let sinc = n == 0 ? 1.0 : sin(.pi * cutoff * n) / (.pi * cutoff * n)
                let hamming = 0.54 + 0.46 * cos(.pi * n / Double(filterTaps))
                return Float(sinc * hamming)
            }
        }

        downsampleCoefficients = (0..<phases).map { phase in
            (0..<filterTaps).map { tap in
                let t = Double(tap) - Double(filterTaps - 1) / 2.0
                let n = t + Double(phase) / Double(phases)
                let sinc = n == 0 ? 1.0 : sin(.pi * cutoff * n) / (.pi * cutoff * n)
    ...
    private func allocateBuffers() {
        upsampleState = (0..<channelCount).map { _ in Array(repeating: 0.0, count: filterTaps) }
        downsampleState = (0..<channelCount).map { _ in Array(repeating: 0.0, count: filterTaps) }
        let maxFrames = 512 // Maximum frame count per callback
        upsampledBuffer = Array(repeating: 0.0, count: maxFrames * upsampleFactor * channelCount)
